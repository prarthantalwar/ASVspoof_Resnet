...
def initParams():
    ...
    parser.add_argument("-f", "--path_to_features", type=str, help="features path",
                        default='/media/serb-s2st/PortableSSD/Resnet/Features/')
    parser.add_argument("-p", "--path_to_protocol", type=str, help="protocol path",
                        default='/home/serb-s2st/workspace/matlab/PRT_SFF_Files/ASVspoof2019_root/LA/ASVspoof2019_LA_cm_protocols/')
    ...
    parser.add_argument('--batch_size', type=int, default=48, help="Mini batch size for training")
...
def train(args):
    ...
    # initialize model
    sffcc_model = ResNet(3, args.enc_dim, resnet_type='18', nclasses=2).to(args.device)
    if args.continue_training:
        sffcc_model = torch.load(os.path.join(args.out_fold, 'anti-spoofing_sffcc_model.pt')).to(args.device)

    sffcc_optimizer = torch.optim.Adam(sffcc_model.parameters(), lr=args.lr,
                                      betas=(args.beta_1, args.beta_2), eps=args.eps, weight_decay=0.0005)

    training_set = ASVspoof2019(args.access_type, args.path_to_features, args.path_to_protocol, 'train',
                                'SFFCC', feat_len=args.feat_len, padding=args.padding)
    ...
    for epoch_num in tqdm(range(args.num_epochs)):
        ...
        adjust_learning_rate(args, sffcc_optimizer, epoch_num)
        if args.add_loss == "ocsoftmax":
        ...
        print('\nEpoch: %d ' % (epoch_num + 1))
        for i, (sffcc, audio_fn, tags, labels) in enumerate(tqdm(trainDataLoader)):
            sffcc = sffcc.unsqueeze(1).float().to(args.device)
            labels = labels.to(args.device)
            feats, sffcc_outputs = sffcc_model(sffcc)
            sffcc_loss = criterion(sffcc_outputs, labels)

            if args.add_loss == "softmax":
                sffcc_optimizer.zero_grad()
                trainlossDict[args.add_loss].append(sffcc_loss.item())
                sffcc_loss.backward()
                sffcc_optimizer.step()

            if args.add_loss == "ocsoftmax":
                ocsoftmaxloss, _ = ocsoftmax(feats, labels)
                sffcc_loss = ocsoftmaxloss * args.weight_loss
                sffcc_optimizer.zero_grad()
            ...
            if args.add_loss == "amsoftmax":
                ...
                sffcc_loss = criterion(moutputs, labels)
                trainlossDict[args.add_loss].append(sffcc_loss.item())
                sffcc_optimizer.zero_grad()
        ...
        with torch.no_grad():
            ...
            for i, (sffcc, audio_fn, tags, labels) in enumerate(tqdm(valDataLoader)):
                sffcc = sffcc.unsqueeze(1).float().to(args.device)
                labels = labels.to(args.device)
                ...
                sffcc_loss = criterion(sffcc_outputs, labels)
                score = F.softmax(sffcc_outputs, dim=1)[:, 0]

                if args.add_loss == "softmax":
                    devlossDict["softmax"].append(sffcc_loss.item())
                elif args.add_loss == "amsoftmax":
                    outputs, moutputs = amsoftmax_loss(feats, labels)
                    sffcc_loss = criterion(moutputs, labels)
                    score = F.softmax(outputs, dim=1)[:, 0]
                    devlossDict[args.add_loss].append(sffcc_loss.item())
        ...
        torch.save(sffcc_model, os.path.join(args.out_fold, 'checkpoint',
                                            'anti-spoofing_sffcc_model_%d.pt' % (epoch_num + 1)))
        if args.add_loss == "ocsoftmax":
        ...
        if val_eer < prev_eer:
            ...
            torch.save(sffcc_model, os.path.join(args.out_fold, 'anti-spoofing_sffcc_model.pt'))
            if args.add_loss == "ocsoftmax":
                loss_model = ocsoftmax
    ...
    return sffcc_model, loss_model


if __name__ == "__main__":
    args = initParams()
    _, _ = train(args)
    model = torch.load(os.path.join(args.out_fold, 'anti-spoofing_sffcc_model.pt'))
    if args.add_loss == "softmax":
        loss_model = None
    else:
